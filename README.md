...
### Noise Sensitivity
#### Description
In legal RAG systems, noise sensitivity measures how frequently the system produces incorrect claims in its response when using either relevant or irrelevant retrieved legal contexts (such as statutes, cases, or interpretations). It assesses the system’s resilience to "noise"—irrelevant or misleading information—and ensures responses remain accurate and grounded in the reference truth. For example, if a user asks about "drunk driving penalties" and retrieved contexts include irrelevant content about "traffic signal violations," noise sensitivity checks if the response incorporates incorrect claims (e.g., conflating drunk driving fines with signal violation penalties) that are unsupported by the reference. High noise sensitivity indicates the system is easily misled by irrelevant contexts, while low sensitivity means it maintains accuracy despite noise.
- Range of values: 0-1 (0 means no incorrect claims in the response; 1 indicates all claims are incorrect due to noise.)
- Whether higher values are better: No. In legal scenarios, lower noise sensitivity is preferred, as it minimizes errors caused by irrelevant or misleading legal information, ensuring the response’s legality and reliability—critical for avoiding misguidance in legal decisions.
